...
int main(int argc, char **argv)
{
    ...
    string* FramesList;
    int frame_count;
    if (!is_video_cache(cutted_video_name, w.ws_row, w.ws_col, fps) || dont_use_cache){
        frame_count = split_video(video_name, cutted_video_name, fps, recreate_frames);
        if (frame_count == -1){
        ...
        for (int k = 0; k<frame_count; k++) {
            ...
            frame = imread(format("%s-%d-opencv/frame%d.jpg",cutted_video_name.c_str(),fps,k));
            Mat resizedFrame;
        ...
        if (create_cache){
            ...
            ofstream fs(format("cache/%s-%d-%d-%d.bin",cutted_video_name.c_str(),w.ws_row,w.ws_col,fps), ios::binary);
        ...
        if (!save_frames){
            bool is_not_created = filesystem::create_directory(format("%s-%d-opencv",cutted_video_name.c_str(),fps));
            if (!is_not_created){
                filesystem::remove_all(format("%s-%d-opencv",cutted_video_name.c_str(),fps));
            }

        }
    }
    else{
        ifstream fs(format("cache/%s-%d-%d-%d.bin",cutted_video_name.c_str(),w.ws_row,w.ws_col,fps), ios::binary);
